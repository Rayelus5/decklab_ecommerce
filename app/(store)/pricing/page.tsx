  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
      ...
      <div className="text-center mb-14">
        ...
        <p className="text-slate-300 mt-3 max-w-lg mx-auto leading-relaxed">
          Cada nivel incluye un allowance mensual para comprar al precio PRO.
          Facturación bimestral — permanencia mínima de 2 meses.
        </p>
      </div>
      ...
      {tiers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {tiers.map((tier, i) => {
            ...
            const isRecommended = i === Math.floor(tiers.length / 2); // tier del centro
              ...
              <tbody className="divide-y divide-white/5">
                ...
                <tr className="hover:bg-white/3 transition-colors">
                  <td className="py-3 px-4 text-slate-300 flex items-center gap-2">
                    <BarChart2 size={13} className="text-slate-300/60" />
                    Allowance mensual
                  </td>
                  {tiers.map((tier) => (
                ...
                <tr className="hover:bg-white/3 transition-colors">
                  <td className="py-3 px-4 text-slate-300 flex items-center gap-2">
                    <Zap size={13} className="text-slate-300/60" />
                    Early access
                  </td>
                  {tiers.map((tier) => {
                ...
                <tr className="hover:bg-white/3 transition-colors">
                  <td className="py-3 px-4 text-slate-300 flex items-center gap-2">
                    <Truck size={13} className="text-slate-300/60" />
                    Envío gratuito
                  </td>
                  {tiers.map((tier) => {
                    const b = (tier.benefits ?? {}) as ProBenefits;
                ...
                <tr className="hover:bg-white/3 transition-colors">
                  <td className="py-3 px-4 text-slate-300 flex items-center gap-2">
                    <Lock size={13} className="text-slate-300/60" />
